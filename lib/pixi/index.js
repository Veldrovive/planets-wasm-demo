...
export function animate (app, viewport, universe) {
    ...
    for (let [id, [sprite, orbitSprite, meta]] of Object.entries(orbitSpriteMap)) {
        ...
        if (cometaryTail) {
            // Create an emitter that will move with the sprite
            const emitter = createEmitter(viewport, 'cometaryTail', {}, false)
            emitter.emit = false
            meta['cometaryEmitter'] = emitter
        }
    }
    ...
    // Set up the camera focus
    // This is used as the point to follow instead of whatever actual object is selected.
    // This is so that was can position the center of screen offset to the object being followed.
    const focus = new PIXI.Sprite(PIXI.Texture.WHITE)
    focus.anchor.set(0.5)
    focus.visible = false
    viewport.addChild(focus)
    ...
    app.ticker.add(delta => {
        ...
        for (let { id, x, y, dir_to_focus } of orbits) {
            if (id != null) {
                // Due to optimization, the id is sometimes undefined if the orbit has been deleted
                const [bodySprite, orbitSprite, meta] = orbitSpriteMap[id]
                if (rateStable) {
                    const { cometaryTail, cometaryEmitter } = meta
                    bodySprite.position.set(x, y)
                    if (cometaryTail) {
                        cometaryEmitter.emit = true
                        cometaryEmitter.rotate(dir_to_focus + Math.PI)
                        cometaryEmitter.updateSpawnPos(x, y)
                        cometaryEmitter.update(delta * 0.01)
                    }
                }
            }
        }
